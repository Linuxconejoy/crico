...
struct GuidedHelpHintSizePreferenceKey: PreferenceKey {
    static var defaultValue: CGSize = .zero
    static func reduce(value: inout CGSize, nextValue: () -> CGSize) {
        value = nextValue()
    }
...
struct BlueCursorView: View {
    ...
    @State private var guidedHelpHintText: String = ""
    @State private var guidedHelpHintOpacity: Double = 0.0
    @State private var guidedHelpHintSize: CGSize = .zero
    @State private var guidedHelpHintScale: CGFloat = 0.96
    ...
    var body: some View {
        ZStack {
            ...
            if buddyNavigationMode == .pointingAtTarget && !guidedHelpHintText.isEmpty {
                Text(guidedHelpHintText)
                    ...
                    .overlay(
                        GeometryReader { geo in
                            Color.clear
                                .preference(key: GuidedHelpHintSizePreferenceKey.self, value: geo.size)
                        }
                    )
                    .scaleEffect(guidedHelpHintScale)
                    .opacity(guidedHelpHintOpacity)
                    .position(
                        x: cursorPosition.x + 10 + (guidedHelpHintSize.width / 2),
                        y: cursorPosition.y + 18 + (navigationBubbleSize.height / 2) + 6 + (guidedHelpHintSize.height / 2)
                    )
                    .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                    .animation(.spring(response: 0.35, dampingFraction: 0.72), value: guidedHelpHintScale)
                    .animation(.easeOut(duration: 0.22), value: guidedHelpHintOpacity)
                    .onPreferenceChange(GuidedHelpHintSizePreferenceKey.self) { newSize in
                        guidedHelpHintSize = newSize
                    }
            }
    ...
    private func startPointingAtElement() {
        ...
        guidedHelpHintText = ""
        guidedHelpHintOpacity = 0.0
        guidedHelpHintSize = .zero
        guidedHelpHintScale = 0.96
        ...
        streamNavigationBubbleCharacter(phrase: pointerPhrase, characterIndex: 0) {
            self.showGuidedHelpHintIfNeeded()
            // All characters streamed — hold for 3 seconds, then fly back
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                guard self.buddyNavigationMode == .pointingAtTarget else { return }
                self.navigationBubbleOpacity = 0.0
                self.guidedHelpHintOpacity = 0.0
    ...
    private func showGuidedHelpHintIfNeeded() {
        guard let overlayHint = companionManager.guidedHelpOverlayHint,
              !overlayHint.isEmpty else {
            return
        }

        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            guard self.buddyNavigationMode == .pointingAtTarget else { return }
            self.guidedHelpHintText = overlayHint
            self.guidedHelpHintScale = 1.0
            self.guidedHelpHintOpacity = 1.0
        }
    }
