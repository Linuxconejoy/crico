...
struct AssemblyAIBackendReadinessStatus {
...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    var isConfigured: Bool {
        AppBundleConfiguration.workerBackendConfigurationStatus.isConfigured
    }

    var unavailableExplanation: String? {
        guard !isConfigured else { return nil }
        return AppBundleConfiguration.workerBackendConfigurationStatus.message
    }
    ...
    static func checkBackendReadiness() async -> AssemblyAIBackendReadinessStatus {
        let provider = AssemblyAIStreamingTranscriptionProvider()
        return await provider.checkBackendReadiness()
    }
    ...
    func startStreamingSession(
        ...
        guard isConfigured else {
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: unavailableExplanation ?? "AssemblyAI transcription is not configured."
            )
        }
    ...
    func checkBackendReadiness() async -> AssemblyAIBackendReadinessStatus {
        let configurationStatus = AppBundleConfiguration.workerBackendConfigurationStatus

        guard configurationStatus.isConfigured else {
            return AssemblyAIBackendReadinessStatus(
                isConfigured: false,
                isReady: false,
        ...
        do {
            _ = try await fetchTemporaryToken()
            return AssemblyAIBackendReadinessStatus(
                isConfigured: true,
                isReady: true,
                message: "Clicky backend reachable. AssemblyAI token endpoint is ready."
            )
        } catch {
            return AssemblyAIBackendReadinessStatus(
                isConfigured: true,
                isReady: false,
                message: "Clicky backend configured, but the AssemblyAI token endpoint is not ready: \(error.localizedDescription)"
            )
        }
    }
    ...
    private func fetchTemporaryToken() async throws -> String {
        guard let workerBackendConfiguration = AppBundleConfiguration.workerBackendConfiguration else {
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: unavailableExplanation ?? "AssemblyAI transcription is not configured."
            )
        }

        var request = URLRequest(url: workerBackendConfiguration.transcribeTokenURL)
        request.httpMethod = "POST"
        if let workerAppKey = workerBackendConfiguration.appKey, !workerAppKey.isEmpty {
            request.setValue(workerAppKey, forHTTPHeaderField: "x-clicky-app-key")
        }
